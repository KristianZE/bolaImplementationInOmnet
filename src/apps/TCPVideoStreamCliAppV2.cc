...
#include "TCPVideoStreamCliAppV2.h"
...
Define_Module(TCPVideoStreamCliAppV2);

simsignal_t TCPVideoStreamCliAppV2::rcvdPkSignal = registerSignal("rcvdPk");
simsignal_t TCPVideoStreamCliAppV2::sentPkSignal = registerSignal("sentPk");

simsignal_t TCPVideoStreamCliAppV2::positionX = registerSignal("positionX");
simsignal_t TCPVideoStreamCliAppV2::positionY = registerSignal("positionY");

TCPVideoStreamCliAppV2::~TCPVideoStreamCliAppV2() {
...
int TCPVideoStreamCliAppV2::generateRandomNumberFromRange(int min, int max) {
    int mean = (max + min) / 2;
...
int TCPVideoStreamCliAppV2::generateRandomizedBitrate(int minBitrate, int maxBitrate) {
...
int TCPVideoStreamCliAppV2::getVideoBitrate(int resolution) {
    if (resolution == 240) {
...
void TCPVideoStreamCliAppV2::nextVidSetup() {
    ...
    saveResultsToFile(true, true, true, true);

}

void TCPVideoStreamCliAppV2::initialize(int stage) {
    ...
    std::string appIdent = std::to_string(getIndex());
    ...
    nodeIdentifier += "app" + appIdent;


    resFileName = "tempBitrateResults/";
    resFileName += configName;
    resFileName += runNumber;
    resFileName += moduleName;
    resFileName += "_app";
    resFileName += appIdent;
    resFileName += ".csv";
    std::fstream fout;
    fout.open(resFileName, std::ios::out | std::ios::trunc);
    fout << "timestamp,bitrate,bufferLevel,playbackPointer,mos\n";
    fout.close();

    useAverage = par("useAvgForLiveMonitoring");
    consideredMs = par("liveMonitSlidingWindow");
...
void TCPVideoStreamCliAppV2::sendRequest() {
    EV << "Send request function. ";
    if (video_buffer <= video_buffer_max_length - segment_length && video_downloading == 0) {
        ...
        EV << "Request will be sent at a later time, as there is not enough space in the buffer or another segment is being downloaded." << endl;
        simtime_t d = simTime() + 1;
        rescheduleOrDeleteTimer(d, MSGKIND_SEND);
    }
}

void TCPVideoStreamCliAppV2::handleTimer(cMessage *msg) {
    ...
    std::string fileName = "configs/VIDconf/";
    fileName += "app";
...
//            saveResultsToFile(false, true, true, true);
            saveResultsToFile(true, true, true, true);
            nextVidSetup();
        } else {
//            saveResultsToFile(false, true, true, false);
            saveResultsToFile(true, true, true, true);
        }
        break;
    default:
...
void TCPVideoStreamCliAppV2::socketEstablished(TcpSocket *socket) { //M: When the socked has been established, send the first request
...
void TCPVideoStreamCliAppV2::rescheduleOrDeleteTimer(simtime_t d, short int msgKind) { //M: Not entirely sure how to describe what is happening here. It probably has something to do with the lifetime of the video client in the simulation
...
void TCPVideoStreamCliAppV2::saveResultsToFile(bool band, bool buffer, bool playback, bool mosScore) {
    std::fstream fout;
    fout.open(resFileName, std::ios::app);
    std::string timeInfo;
    ...
    if (useAverage) {
        int64_t sumBits = 0;
        simtime_t minTime = curTime - SimTime(consideredMs,SIMTIME_MS); // Only consider values from the last consideredMs miliseconds.
        for (auto it = rcvdBitsStore.cbegin(), next_it = it; it != rcvdBitsStore.cend(); it = next_it) {
            ++next_it;
            if (it->first > minTime) { // Sum bits if received within the last consideredMs ms
                sumBits += it->second;
            } else { // Otherwise erase from map
                rcvdBitsStore.erase(it);
            }
        }
        avgBandwidth = sumBits*1000 / consideredMs; // We are gathering bits from the last second, hence the division by consideredMs converted to seconds.
    } else {
        if (downloadTime != 0) {
            avgBandwidth = (double) lastCheckBits / downloadTime;
        }
    }
    if (band) {
    ...
    timeInfo += ",";
    if (buffer) {
        timeInfo += std::to_string(video_buffer);
    }
    timeInfo += ",";
    if (playback) {
        timeInfo += std::to_string(video_playback_pointer);
    }
    timeInfo += ",";
    if (mosScore) {
        timeInfo += std::to_string(mos);
    }
    timeInfo += "\n";
    fout << timeInfo;
    fout.close();
...
void TCPVideoStreamCliAppV2::socketDataArrived(TcpSocket *socket, Packet *msg, bool urgent) {
    ...
    if (useAverage) {
        rcvdBitsStore[curTime] = msg->getBitLength() + 320;
    } else {
    ...
    // If there was no bitrate update for quite some time, do the update
    double downloadTime = ((double) curTime.raw() - (double) lastCheckTime.raw()) / (double) pow(10,12);
    if (downloadTime >= 0.9) {
        saveResultsToFile(true, true, true, true);
    }
...
std::string TCPVideoStreamCliAppV2::getVResString(int resolution) {
    if (resolution == 240) {
        return "426x240";
    } else if (resolution == 360) {
        return "640x360";
...
void TCPVideoStreamCliAppV2::prepareCSV(void) {
    std::fstream fout;
...
double TCPVideoStreamCliAppV2::calculateMOS(void) {
    // Prepare csv file used for MOS calculation
    TCPVideoStreamCliAppV2::prepareCSV(); // Will prepare a csv file with the collected information
...
void TCPVideoStreamCliAppV2::socketClosed(TcpSocket *socket) {
    EV << "Socket closed. Connection id: " << socket->getSocketId() << "\n";   //EDIT!
    TcpAppBase::socketClosed(socket);                      //EDIT!
    video_downloading = 0;
}

void TCPVideoStreamCliAppV2::socketFailure(TcpSocket *socket, int code) {
...
void TCPVideoStreamCliAppV2::refreshDisplay() const
{
    char buf[64];
